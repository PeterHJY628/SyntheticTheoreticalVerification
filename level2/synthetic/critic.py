"""Spectral-norm IPM critic for Level 2."""
...
def train_and_evaluate_critic(
    ...
    critic = SpectralNormLayerIPM(input_dim, config.hidden_dim).to(x_l.device)
    torch.manual_seed(seed)
    train_ipm_critic(critic, x_l, p_l.detach(), x_u, p_u.detach(), config)
    ipm_value = evaluate_ipm(critic, x_l, p_l, x_u, p_u, config.population_batch_size)
    return critic, ipm_value
